...
export class CatalogComponent {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe((params) => {
      this.filter = params["filter"] ?? ''; // subscription (params) publishes a params object. Check if it contains a filter or not.
    });
  }
  ...
  addToCart(product: IProduct) {
    ...
    this.router.navigate(["/cart"]); // Goes to cart after clicking an item
  }
}
